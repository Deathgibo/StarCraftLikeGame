...
class BSPNode():
    ...
    def f(self, pindex, graph):
        return (self.n[0] * (graph.vtable[pindex].pos[0] - graph.vtable[self.p1index].pos[0])) + (
                    self.n[1] * (graph.vtable[pindex].pos[1] - graph.vtable[self.p1index].pos[1]))
    ...
    def fpos(self, p, graph, bsp):
        ...
        return (self.n[0] * (p[0] - graph.vtable[self.p1index].pos[0])) + (
                    self.n[1] * (p[1] - graph.vtable[self.p1index].pos[1]))
...
class BSP():
    ...
    def printindex(self, i, display_render, map):
        ...
        color = (0, 0, 0)
        pygame.draw.line(display_render, color, (
        self.graph.vtable[self.tree[i].p1index].pos[0] - map._cameraposition[0],
        self.graph.vtable[self.tree[i].p1index].pos[1] - map._cameraposition[1]),
                         (self.graph.vtable[self.tree[i].p2index].pos[0] - map._cameraposition[0],
                          self.graph.vtable[self.tree[i].p2index].pos[1] - map._cameraposition[1]))
        l = 50
    ...
    def calculatenormal(self, p1, p2):
        vec = (self.graph.vtable[p1].pos[0] - self.graph.vtable[p2].pos[0],
               self.graph.vtable[p1].pos[1] - self.graph.vtable[p2].pos[1])
