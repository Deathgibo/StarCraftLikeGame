...
class Entity():
    def __init__(self, rad, imgsurf, pyrect):
        ...
        self.circlecenter = np.array(
            [int(pyrect.x + self.circlexpercent * pyrect.w), int(pyrect.y + self.circleypercent * pyrect.h)])
        # render
    ...
    def render(self, game):
        ...
        camerarect = pygame.Rect(game.map._cameraposition[0], game.map._cameraposition[1],
                                 game.map._renderdimensions[0], game.map._renderdimensions[1])
        ...
        rectangle = pygame.Rect(int(round(self.healthrect[0])) - game.map._cameraposition[0],
                                int(round(self.healthrect[1])) - game.map._cameraposition[1],
    ...
    def Move(self, x, y, enemyunits=None, units=None, enemyunitsquad=None, unitsquad=None, z=None):
        ...
        if enemyunits != None and units != None:
            ...
            if unit is not None:
                movevector = np.array(
                    [unit.circlecenter[0] - self.circlecenter[0], unit.circlecenter[1] - self.circlecenter[1]])
                movevector = movevector / np.linalg.norm(movevector)
                movevector = movevector * 5
                z.append(unit)
        ...
        self.circlecenter = np.array(
            [self.rect[0] + self.circlexpercent * self.rect[2], self.rect[1] + self.circleypercent * self.rect[3]])
    ...
    def statetrigger(self, input, unit, minerals, mapinfo, displaysurf, enemyunits, enemyunitsquad, unitsquad):
        ...
        if unit.selected:
            ...
            if input.rightclickframe:
                worldcoords = mapinfo.windowtoworldtransform(input.mouseclickposition[0], input.mouseclickposition[1],
                                                             displaysurf)
                if unit.worker:  # check if clicked on a resource
                    for mineral in minerals:
                        if mathfuncs.mathfuncs.circlecirclecollision(
                                (mineral.circlecenter[0], mineral.circlecenter[1], mineral.radius),
    ...
    def searchenemies(self, enemylist, enemyunitsquad):
        ...
        l = []
        enemy = enemyunitsquad.PointsInCircleStart((self.circlecenter[0], self.circlecenter[1], self.visionrange),
                                                   enemylist, l)
...
class miningstate(entity_state):
    ...
    def update(self, unit, enemyunits=None, units=None, enemyunitsquad=None, unitsquad=None):
        if self.state == 0:
            ...
            if mathfuncs.mathfuncs.circlecirclecollision((unit.circlecenter[0], unit.circlecenter[1], unit.radius),
                                                         (unit.mineralpatch.circlecenter[0],
                                                          unit.mineralpatch.circlecenter[1], unit.mineralpatch.radius)):
                self.stateone(unit)
    ...
    def statezero(self, unit):
        self.state = 0
        self.dir = np.array([unit.mineralpatch.circlecenter[0] - unit.circlecenter[0],
                             unit.mineralpatch.circlecenter[1] - unit.circlecenter[1]])
...
class movestate(entity_state):
    ...
    def entry(self, unit, graph):
        ...
        if len(self.movepaths) > 0:
            ...
            self.dir = np.array(
                [unit.movelocation[0] - unit.circlecenter[0], unit.movelocation[1] - unit.circlecenter[1]])
            self.dir = self.dir / np.linalg.norm(self.dir)
...
class attackstate(entity_state):
    ...
    def update(self, unit, enemyunits=None, units=None, enemyunitsquad=None, unitsquad=None):
        ...
        if not self.attacking:
            targetdistance = mathfuncs.mathfuncs.Magnitude((unit.circlecenter[0] - unit.target.circlecenter[0],
                                                            unit.circlecenter[1] - unit.target.circlecenter[1]))
            if targetdistance > unit.attackrange:
                self.attackchase = True
                if not self.followtarget:
                    enemylist = []
                    l = []
                    enemy = enemyunitsquad.PointsInCircleStart(
                        (unit.circlecenter[0], unit.circlecenter[1], targetdistance), enemylist, l)
                    if enemy is not None:
                        unit.target = enemy
                        if mathfuncs.mathfuncs.Magnitude((unit.circlecenter[0] - enemy.circlecenter[0],
                                                          unit.circlecenter[1] - enemy.circlecenter[
                                                              1])) <= unit.attackrange:
                            self.attackchase = False
        ...
        if self.attackchase:
            self.dir = np.array([unit.target.circlecenter[0] - unit.circlecenter[0],
                                 unit.target.circlecenter[1] - unit.circlecenter[1]])
            self.dir = self.dir / np.linalg.norm(self.dir)
            unit.Move(self.dir[0] * unit.speed, self.dir[1] * unit.speed, enemyunits, units, enemyunitsquad, unitsquad)
        else:
            if unit.attackspeedcounter == unit.attackspeed:
