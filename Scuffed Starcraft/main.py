...
class App():
    ...
    def on_update(self):
        ...
        self._playerinfo.update(self._input, self._entitylist, self.map, self._display_surf, self._building_list,
                                self._enemyentitylist,
        ...
        for units in self._entitylist:
            units.update(self._input, self.minerallist, self.map, self._display_surf, self._enemyentitylist,
                         self._entitylist, self._enemyentityquadtree, self._entityquadtree,
        ...
        for units in self._enemyentitylist:
            units.update(self._input, self.minerallist, self.map, self._display_surf, self._entitylist,
                         self._enemyentitylist, self._entityquadtree, self._enemyentityquadtree,
                         self.worldgraph, self._playerinfo, self._building_list)
    ...
    def on_render(self):
        ...
        for x in self._entitylist:
            x.render(self, (0, 0, 255))

        # enemy units
        for x in self._enemyentitylist:
            x.render(self, (255, 0, 0))
        ...
        resized_screen = pygame.transform.scale(self._display_surfrender,
        ...
        if self.overlay_enable:
            ...
            self.overlay.renderSelectedItem(self._display_surf, self._playerinfo._selectedlist,
                                            self._playerinfo._selectedbuildinglist)
    ...
    def help(self):
        ...
        while True:
            ...
            self._display_surf.blit(title, (250, 0))

            # Menu Options
            # pygame.draw.rect(screen, [red, green, blue], [left, top, width, height], filled)
            top_rect = pygame.draw.rect(self._display_surf, (0, 0, 255), (240, 144, 380, 35))
            bottom_rect = pygame.draw.rect(self._display_surf, (0, 0, 255), (280, 195, 240, 35))
            self.draw_rect((0, 0, 255), (255, 255, 255), top_rect, 2)
            self.draw_rect((0, 0, 255), (255, 255, 255), bottom_rect, 2)
            self.draw_text("PRESS [ENTER] TO CONTINUE", 24, 245, 149, (255, 255, 255))
            self.draw_text("PRESS [Q] TO QUIT", 24, 285, 200, (215, 255, 255))

            inst_rec = pygame.draw.rect(self._display_surf, (0, 0, 255), (10, 250, 770, 350))
            self.draw_rect((0, 0, 255), (255, 255, 255), inst_rec, 2)

            self.draw_text("-The left mouse button is used to select units, buildings, command ",
                           20, 20, 260, (255, 255, 255))
            self.draw_text("buttons and points of action (locations where orders are carried out).",
                           20, 20, 290, (255, 255, 255))
            self.draw_text("-Right click moves units and they won’t attack until they reach that position. ",
                           20, 20, 320, (255, 255, 255))
            self.draw_text("A + right click will attack move them. ‘S’ will stop them. A + left clicking a",
                           20, 20, 350, (255, 255, 255))
            self.draw_text("unit will make them attack and follow that specific unit. ",
                           20, 20, 380, (255, 255, 255))
            self.draw_text("-Press 2 to zoom in, and 1 to zoom back out.",
                           20, 20, 410, (255, 255, 255))
            self.draw_text("-Use Workers to collect resources to bring to base. Uses Marines and ",
                           20, 20, 440, (255, 255, 255))
            self.draw_text("Marauders to attack.",
                           20, 20, 470, (255, 255, 255))
            self.draw_text("-Left click buildings to choose what troops to spawn in the bottom right.",
                           20, 20, 500, (255, 255, 255))
            self.draw_text("-Press ‘-’ to toggle UI.",
                           20, 20, 530, (255, 255, 255))
            self.draw_text("-Objective: Destroy the enemy units and bases.",
                           20, 20, 560, (255, 255, 255))
    ...
    def on_event(self, event):
        ...
        if event.type == QUIT:
            ...
            if event.button == 1:  # l
                ...
                self.overlay.buttonHandle(self._display_surf, self._input, self._playerinfo, self._entitylist,
                                          self._entityquadtree, self._enemyentitylist, self._enemyentityquadtree)
