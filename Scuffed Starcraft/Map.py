...
class Map:
    ...
    def handleinput(self, input, displaysurf, mouseimg, mouseimglist,
                    editmode):  # returns tuple(mouseimgvalue, mousexoffset,mouseyoffset)
        ...
        if not editmode:
            if self.ispointonminimap(input.mouseclickposition[0], input.mouseclickposition[1], displaysurf):
                if input.leftclick == True:
                    ...
                    self._cameraposition[0] = int(self._mapimage.get_width() * xpercent) - (
                                self._renderdimensions[0] / 2)
                    self._cameraposition[1] = int(self._mapimage.get_height() * ypercent) - (
                                self._renderdimensions[1] / 2)
        ...
        if self._cameraposition[1] > self._mapimage.get_height() - self._renderdimensions[
            1]:  # displaysurf.get_height()
    ...
    def render(self, displaysurf):
        ...
        rectclip = pygame.Rect(self._cameraposition[0], self._cameraposition[1], self._renderdimensions[0] - widthclip,
                               self._renderdimensions[1] - heightclip)
    ...
    def windowtoworldtransform(self, x, y, displaysurf):
        ...
        return (self._cameraposition[0] + (xpercent * self._renderdimensions[0]),
                self._cameraposition[1] + (ypercent * self._renderdimensions[1]))
