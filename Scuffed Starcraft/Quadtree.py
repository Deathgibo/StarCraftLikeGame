        ...
        if slot[0] == 0:
            if node.bottomleft is not None:
                ...
                if isleaf and len(node.info) == 0:
                    ...
                    node.bottomleft = Quadnode(
                        (node.square[0], node.square[1], node.square[2] / 2.0, node.square[3] / 2.0))
        ...
        while not q.empty():
            ...
            if top[0] is None:
                ...
                if len(top[0].info) == 0:
                    ...
                    print("(leaf, %f, %f, %f, %s)" % (
                    top[0].info[0].circlecenter[0], top[0].info[0].circlecenter[1], top[1], top[2]))
    ...
    def PointsInBox(self, node, box, l, inside):
        if node.isleaf():
            if inside:
                ...
                for x in range(0, len(node.info)):
                    if mathfuncs.mathfuncs.rectcirclecollision(box[0], box[1], box[2], box[3],
                                                               node.info[x].circlecenter[0],
                                                               node.info[x].circlecenter[1], node.info[x].radius):
                        l.append(node.info[x])
    ...
    def PointsInCircle(self, node, circle, l, inside, unitrepeat):
        if node.isleaf():
            if inside:
                ...
                for x in range(0, len(node.info)):
                    if mathfuncs.mathfuncs.pointcirclecollision(node.info[x].circlecenter, circle) and node.info[
                        x] not in unitrepeat:
                        return node.info[x]
                        """if len(unitrepeat) == 0:
                            return node.info[x]
        ...
        if node.bottomleft is not None:
            ...
            if mathfuncs.mathfuncs.rectcirclecollision(node.bottomleft.square[0], node.bottomleft.square[1],
                                                       node.bottomleft.square[2], node.bottomleft.square[3], circle[0],
                                                       circle[1], circle[2]):
                unit = self.PointsInCircle(node.bottomleft, circle, l,
                                           mathfuncs.mathfuncs.boxincircle(node.bottomleft.square[0],
                                                                           node.bottomleft.square[1],
                                                                           node.bottomleft.square[2],
                                                                           node.bottomleft.square[3], circle[0],
                                                                           circle[1], circle[2]), unitrepeat)
                if unit is not None:
                    return unit
        ...
        if node.topleft is not None:
            ...
            if mathfuncs.mathfuncs.rectcirclecollision(node.topleft.square[0], node.topleft.square[1],
                                                       node.topleft.square[2], node.topleft.square[3], circle[0],
                                                       circle[1], circle[2]):
                unit = self.PointsInCircle(node.topleft, circle, l,
                                           mathfuncs.mathfuncs.boxincircle(node.topleft.square[0],
                                                                           node.topleft.square[1],
                                                                           node.topleft.square[2],
                                                                           node.topleft.square[3], circle[0], circle[1],
                                                                           circle[2]), unitrepeat)
                if unit is not None:
                    return unit
        ...
        if node.topright is not None:
            ...
            if mathfuncs.mathfuncs.rectcirclecollision(node.topright.square[0], node.topright.square[1],
                                                       node.topright.square[2], node.topright.square[3], circle[0],
                                                       circle[1], circle[2]):
                unit = self.PointsInCircle(node.topright, circle, l,
                                           mathfuncs.mathfuncs.boxincircle(node.topright.square[0],
                                                                           node.topright.square[1],
                                                                           node.topright.square[2],
                                                                           node.topright.square[3], circle[0],
                                                                           circle[1], circle[2]), unitrepeat)
                if unit is not None:
                    return unit
        ...
        if node.bottomright is not None:
            ...
            if mathfuncs.mathfuncs.rectcirclecollision(node.bottomright.square[0], node.bottomright.square[1],
                                                       node.bottomright.square[2], node.bottomright.square[3],
                                                       circle[0], circle[1], circle[2]):
                unit = self.PointsInCircle(node.bottomright, circle, l,
                                           mathfuncs.mathfuncs.boxincircle(node.bottomright.square[0],
                                                                           node.bottomright.square[1],
                                                                           node.bottomright.square[2],
                                                                           node.bottomright.square[3], circle[0],
                                                                           circle[1], circle[2]), unitrepeat)
                if unit is not None:
                    return unit
    ...
    def PointslistInCircle(self, node, circle, l, inside):
        ...
        if node.bottomleft is not None:
            ...
            if mathfuncs.mathfuncs.rectcirclecollision(node.bottomleft.square[0], node.bottomleft.square[1],
                                                       node.bottomleft.square[2], node.bottomleft.square[3], circle[0],
                                                       circle[1], circle[2]):
                self.PointslistInCircle(node.bottomleft, circle, l,
                                        mathfuncs.mathfuncs.boxincircle(node.bottomleft.square[0],
                                                                        node.bottomleft.square[1],
                                                                        node.bottomleft.square[2],
                                                                        node.bottomleft.square[3], circle[0], circle[1],
                                                                        circle[2]))
        ...
        if node.topleft is not None:
            ...
            if mathfuncs.mathfuncs.rectcirclecollision(node.topleft.square[0], node.topleft.square[1],
                                                       node.topleft.square[2], node.topleft.square[3], circle[0],
                                                       circle[1], circle[2]):
                self.PointslistInCircle(node.topleft, circle, l,
                                        mathfuncs.mathfuncs.boxincircle(node.topleft.square[0], node.topleft.square[1],
                                                                        node.topleft.square[2], node.topleft.square[3],
                                                                        circle[0], circle[1], circle[2]))
        ...
        if node.topright is not None:
            ...
            if mathfuncs.mathfuncs.rectcirclecollision(node.topright.square[0], node.topright.square[1],
                                                       node.topright.square[2], node.topright.square[3], circle[0],
                                                       circle[1], circle[2]):
                self.PointslistInCircle(node.topright, circle, l,
                                        mathfuncs.mathfuncs.boxincircle(node.topright.square[0],
                                                                        node.topright.square[1],
                                                                        node.topright.square[2],
                                                                        node.topright.square[3], circle[0], circle[1],
                                                                        circle[2]))
        ...
        if node.bottomright is not None:
            ...
            if mathfuncs.mathfuncs.rectcirclecollision(node.bottomright.square[0], node.bottomright.square[1],
                                                       node.bottomright.square[2], node.bottomright.square[3],
                                                       circle[0], circle[1], circle[2]):
                self.PointsInCircle(node.bottomright, circle, l,
                                    mathfuncs.mathfuncs.boxincircle(node.bottomright.square[0],
                                                                    node.bottomright.square[1],
                                                                    node.bottomright.square[2],
                                                                    node.bottomright.square[3], circle[0], circle[1],
                                                                    circle[2]))
